...
def build_map(df, commodity_name, month_num, price_type):
    ...
    # Normalise KoBo region names to canonical GeoJSON names
    ...
    m = folium.Map(location=[7.9465, -1.0232], zoom_start=6.4,
    ...
    def price_to_color(price):
        if max_p == min_p:
            return "#ffdd57"
        ratio = (price - min_p) / (max_p - min_p)  # 0=cheapest→green, 1=expensive→red
    ...
    # Enrich GeoJSON with price so tooltip can show it
    ...
    def style_fn(feature):
        ...
        if price is None:
            return {"fillColor":"#d1d5db","color":"#9ca3af","weight":1,"fillOpacity":0.5}
        return {"fillColor": price_to_color(price), "color": "#555",
                "weight": 1.5, "fillOpacity": 0.80}

    def highlight_fn(feature):
        return {"weight": 3, "color": "#1e293b", "fillOpacity": 0.95}
    ...
    folium.GeoJson(
        ...
        tooltip=folium.GeoJsonTooltip(
            fields=["region", "price_label"],
            aliases=["Region:", "Avg price:"],
            style="font-size:13px;",
        ),
    ).add_to(m)

    # Price labels at region centroids
    for feat in geojson_copy["features"]:
        r = feat["properties"]["region"]
        p = price_map.get(r)
        if p is None:
...
with tab2:
    ...
    with st.expander("🔎 Region name debug (collapse once map works)", expanded=False):
        kobo_regions = map_sub["region"].dropna().unique().tolist()
        st.write("**KoBo region values in data:**", sorted(kobo_regions))
        st.write("**GeoJSON canonical names:**",
                 sorted([f["properties"]["region"] for f in GHANA_GEOJSON["features"]]))
        if not matched.empty:
            st.success("**Matched:** " + str(sorted(matched["region_canon"].unique().tolist())))
        else:
            st.error("No regions matched — KoBo names don't align with GeoJSON names above.")
