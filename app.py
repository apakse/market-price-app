...
GHANA_GEOJSON = {
  ...
  "features": [
    {"type":"Feature","properties":{"region":"Greater Accra"},
     "geometry":{"type":"Polygon","coordinates":[[[-0.55,5.55],[-0.55,5.90],[0.10,5.90],[0.10,5.55],[-0.55,5.55]]]}},
    {"type":"Feature","properties":{"region":"Ashanti"},
     "geometry":{"type":"Polygon","coordinates":[[[-2.70,6.00],[-2.70,7.40],[-1.00,7.40],[-1.00,6.00],[-2.70,6.00]]]}},
    {"type":"Feature","properties":{"region":"Western"},
     "geometry":{"type":"Polygon","coordinates":[[[-3.20,4.75],[-3.20,6.50],[-1.80,6.50],[-1.80,4.75],[-3.20,4.75]]]}},
    {"type":"Feature","properties":{"region":"Central"},
     "geometry":{"type":"Polygon","coordinates":[[[-1.80,5.00],[-1.80,6.20],[-0.55,6.20],[-0.55,5.00],[-1.80,5.00]]]}},
    {"type":"Feature","properties":{"region":"Eastern"},
     "geometry":{"type":"Polygon","coordinates":[[[-1.00,5.80],[-1.00,7.00],[0.15,7.00],[0.15,5.80],[-1.00,5.80]]]}},
    {"type":"Feature","properties":{"region":"Volta"},
     "geometry":{"type":"Polygon","coordinates":[[[0.10,5.55],[0.10,8.60],[1.18,8.60],[1.18,5.55],[0.10,5.55]]]}},
    {"type":"Feature","properties":{"region":"Oti"},
     "geometry":{"type":"Polygon","coordinates":[[[-0.10,7.80],[-0.10,9.00],[1.18,9.00],[1.18,7.80],[-0.10,7.80]]]}},
    {"type":"Feature","properties":{"region":"Bono"},
     "geometry":{"type":"Polygon","coordinates":[[[-3.20,7.20],[-3.20,8.40],[-1.50,8.40],[-1.50,7.20],[-3.20,7.20]]]}},
    {"type":"Feature","properties":{"region":"Bono East"},
     "geometry":{"type":"Polygon","coordinates":[[[-1.50,7.40],[-1.50,8.60],[-0.10,8.60],[-0.10,7.40],[-1.50,7.40]]]}},
    {"type":"Feature","properties":{"region":"Ahafo"},
     "geometry":{"type":"Polygon","coordinates":[[[-3.00,6.80],[-3.00,7.60],[-2.00,7.60],[-2.00,6.80],[-3.00,6.80]]]}},
    {"type":"Feature","properties":{"region":"Northern"},
     "geometry":{"type":"Polygon","coordinates":[[[-2.80,9.00],[-2.80,10.70],[-0.15,10.70],[-0.15,9.00],[-2.80,9.00]]]}},
    {"type":"Feature","properties":{"region":"Savannah"},
     "geometry":{"type":"Polygon","coordinates":[[[-2.80,8.40],[-2.80,9.50],[-1.20,9.50],[-1.20,8.40],[-2.80,8.40]]]}},
    {"type":"Feature","properties":{"region":"North East"},
     "geometry":{"type":"Polygon","coordinates":[[[-0.15,9.50],[-0.15,10.70],[1.00,10.70],[1.00,9.50],[-0.15,9.50]]]}},
    {"type":"Feature","properties":{"region":"Upper East"},
     "geometry":{"type":"Polygon","coordinates":[[[-1.20,10.50],[-1.20,11.15],[1.00,11.15],[1.00,10.50],[-1.20,10.50]]]}},
    {"type":"Feature","properties":{"region":"Upper West"},
     "geometry":{"type":"Polygon","coordinates":[[[-3.00,9.80],[-3.00,11.15],[-1.20,11.15],[-1.20,9.80],[-3.00,9.80]]]}},
    {"type":"Feature","properties":{"region":"Western North"},
     "geometry":{"type":"Polygon","coordinates":[[[-3.20,6.40],[-3.20,7.40],[-2.20,7.40],[-2.20,6.40],[-3.20,6.40]]]}},
  ]
}
...
def tidy_data(df):
    ...
    df = df.melt(id_vars=id_cols, value_vars=avg_cols, var_name="commodity", value_name="Price")
...
def predict_next_month(df, commodity_name, price_type="retail"):
    sub = df[(df["commodity_name"] == commodity_name) & (df["Type"] == price_type)].copy()
    if sub.empty or "market_day" not in sub.columns:
        return None, None, None, "insufficient data"

    monthly = (sub.groupby(["Year","Month"])["Price"]
    ...
    if len(monthly) < 2:
        ...
        slope = (monthly["Price"].iloc[-1] - monthly["Price"].iloc[0]) / max(len(monthly)-1, 1)
        pred  = last + slope
...
def build_map_html(df, commodity_name, month_num, price_type):
    """Returns a self-contained HTML string with a Leaflet choropleth map."""
    sub = df[
    ...
    sub = sub[sub["region_canon"].notna()]
    region_prices = sub.groupby("region_canon")["Price"].mean().reset_index()
    ...
    # Enrich GeoJSON copy with price data
    geojson_copy = json.loads(json.dumps(GHANA_GEOJSON))
    for feat in geojson_copy["features"]:
        r = feat["properties"]["region"]
        p = price_map.get(r)
        feat["properties"]["price"] = round(p, 2) if p else None
        feat["properties"]["price_label"] = f"GH₵ {p:,.0f}" if p else "No data"

    geojson_str = json.dumps(geojson_copy)
...
<style>
  ...
  .info {{
    ...
    box-shadow:0 1px 5px rgba(0,0,0,.3);font-family:sans-serif;font-size:13px;
    line-height:1.6;color:#111;min-width:160px;
  }}
  .legend {{
    background:white;padding:10px 14px;border-radius:8px;
    box-shadow:0 1px 5px rgba(0,0,0,.3);font-family:sans-serif;
    font-size:12px;line-height:1.8;color:#111;
  }}
  .legend-bar {{
...
<div id="map"></div>
<script>
var map = L.map('map', {{zoomControl:true}}).setView([7.9, -1.0], 6);
...
function priceToColor(price) {{
  if (maxP === minP) return '#facc15';
  var ratio = (price - minP) / (maxP - minP);
  var r = Math.round(34  + ratio * (220 - 34));
  var g = Math.round(197 - ratio * (197 - 38));
  var b = Math.round(94  - ratio * (94  - 38));
  return 'rgb(' + r + ',' + g + ',' + b + ')';
}}

function style(feature) {{
  var p = feature.properties.price;
  return {{
    fillColor: p !== null ? priceToColor(p) : '#d1d5db',
    weight: 1.5,
    color: '#555',
    fillOpacity: p !== null ? 0.82 : 0.4
  }};
}}
...
info.update = function(props) {{
  this._div.innerHTML = props
    ? '<b>' + props.region + '</b><br/>' + props.price_label
    : 'Hover over a region';
}};
info.addTo(map);
...
function highlightFeature(e) {{
  var layer = e.target;
  layer.setStyle({{weight:3,color:'#1e293b',fillOpacity:0.95}});
  layer.bringToFront();
...
var geojson = L.geoJson({geojson_str}, {{
  style: style,
...
geojsonData.features.forEach(function(feat) {{
  if (feat.properties.price === null) return;
  ...
  L.marker([cy,cx],{{
    icon: L.divIcon({{
      html: '<div style="font-size:10px;font-weight:700;color:#111;text-align:center;'
          + 'text-shadow:0 0 3px #fff,0 0 3px #fff,0 0 3px #fff;white-space:nowrap;">'
          + 'GH₵' + feat.properties.price.toLocaleString() + '</div>',
      className: '',
      iconSize: [80,18],
      iconAnchor: [40,9]
    }})
...
legend.onAdd = function() {{
  ...
  div.innerHTML = '<b>{commodity_name}</b><br/>'
    + '<div class="legend-bar"></div>'
    + '<span style="float:left">GH₵{min_p:.0f} cheapest</span>'
    + '<span style="float:right">GH₵{max_p:.0f} priciest</span>'
    + '<br style="clear:both"/>';
  return div;
}};
legend.addTo(map);
...
with st.sidebar:
    ...
    if has_secrets:
        ...
        password = st.text_input("KoBo Password", type="password", placeholder="password")
    ...
    if not username or not password:
        ...
        for i, (label, aid) in enumerate(DEFAULT_ASSETS.items()):
            ...
            try:
                ...
                st.error(f"❌ HTTP {e.response.status_code} fetching {label}. Check credentials.")
...
with tab1:
    ...
    with fc2:
        commodities = st.multiselect("Commodity",
    ...
    filtered = df.copy()
    if price_type:  filtered = filtered[filtered["Type"].isin(price_type)]
    if commodities: filtered = filtered[filtered["commodity_name"].isin(commodities)]
    if regions and "region" in filtered.columns:
        filtered = filtered[filtered["region"].isin(regions)]

    st.caption(f"Showing **{len(filtered):,}** of {len(df):,} records")

    display_cols = [c for c in ["market_day","Year","Month","rWeek","week","region",
                                 "district","Market","commodity_number","commodity_name",
                                 "Price","Type","name.officer"] if c in filtered.columns]
    st.dataframe(filtered[display_cols].sort_values("market_day",ascending=False)
                 .head(500).reset_index(drop=True),
                 use_container_width=True, height=380)

    if commodities and "market_day" in filtered.columns:
        import altair as alt
        chart_data = (filtered.groupby(["market_day","commodity_name","Type"])["Price"]
                      .mean().reset_index())
        chart = (alt.Chart(chart_data).mark_line(point=True)
                 .encode(x=alt.X("market_day:T", title="Market day"),
                         y=alt.Y("Price:Q", title="Avg price (GH₵)"),
                         color="commodity_name:N", strokeDash="Type:N",
...
with tab2:
    ...
    with mc4:
        map_year = st.selectbox("Year", available_years,
                                index=len(available_years)-1, key="map_year")
    ...
    map_sub["region_canon"] = map_sub["region"].apply(
        lambda r: GEOJSON_REGION_NORM.get(normalise_region(r), None)
    )
    matched = map_sub[map_sub["region_canon"].notna()]

    if matched.empty:
        st.warning(f"No regional data for **{map_commodity}** in "
                   f"**{MONTH_NAMES[map_month_num]} {map_year}** ({map_type}). "
                   f"Check the debug panel above for region name mismatches.")
    else:
        map_html, price_map = build_map_html(map_df, map_commodity, map_month_num, map_type)
        if map_html:
            import streamlit.components.v1 as components
            components.html(map_html, height=500, scrolling=False)
        else:
            st.info("No price data to display on map for this selection.")
    ...
    if status == "ok" and pred_price is not None:
        ...
        p2.metric(f"Predicted — {next_month_name} (GH₵)",
                  f"{pred_price:,.2f}",
        ...
        monthly_hist = (sub.groupby(["Year","Month"])["Price"]
                        .mean().reset_index().sort_values(["Year","Month"]))
        if not monthly_hist.empty:
            ...
            pred_row = pd.DataFrame([{
                "Year": monthly_hist["Year"].iloc[-1] + (1 if next_month==1 else 0),
                "Month": next_month, "Price": pred_price,
                "date_label": f"{next_month_name} (pred)", "t": len(monthly_hist)
            }])
            ...
            line = (base.mark_line()
                    .encode(x=alt.X("t:Q", axis=alt.Axis(labels=False), title=""),
                            y=alt.Y("Price:Q", title="Avg price (GH₵)"),
                            color=alt.condition(
            ...
            points = (base.mark_point(size=60)
                      .encode(x="t:Q", y="Price:Q",
                              color=alt.condition(
                                  alt.datum.is_pred, alt.value("#ef4444"), alt.value("#3b82f6")),
                              tooltip=["date_label:N", alt.Tooltip("Price:Q", format=".0f")]))
            st.altair_chart((line + points).properties(
                height=220, title=f"{map_commodity} ({map_type}) — historical & prediction"
...
with tab3:
    ...
    with ec1:
        exp_type = st.multiselect("Price type",
                                   df["Type"].dropna().unique().tolist(),
                                   default=df["Type"].dropna().unique().tolist(),
                                   key="exp_type")
    with ec2:
        exp_comm = st.multiselect("Commodity (blank = all)",
                                   sorted(df["commodity_name"].dropna().unique().tolist()),
                                   key="exp_comm")
    with ec3:
        exp_reg = st.multiselect("Region (blank = all)",
                                  sorted(df["region"].dropna().unique().tolist())
                                  if "region" in df.columns else [],
                                  key="exp_reg")

    exp_df = df.copy()
    if exp_type: exp_df = exp_df[exp_df["Type"].isin(exp_type)]
    if exp_comm: exp_df = exp_df[exp_df["commodity_name"].isin(exp_comm)]
    ...
    with dl1:
        st.download_button("⬇️ Download CSV", data=exp_df.to_csv(index=False).encode(),
    ...
    with dl2:
        ...
        with pd.ExcelWriter(xl, engine="openpyxl") as w:
            ...
            exp_df[exp_df["Type"]=="retail"].to_excel(w, index=False, sheet_name="Retail")
            exp_df[exp_df["Type"]=="wholesale"].to_excel(w, index=False, sheet_name="Wholesale")
