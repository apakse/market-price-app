...
# ── Page config ───────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="MOFA SRID Live Price Monitor",
    page_icon="🫘🌶️🥩🌽",
    layout="wide",
...
with st.sidebar:
    ...
    download_btn = st.button(
        "⬇️  Fetch & Refresh Data", use_container_width=True, type="primary"
    )
...
if "df" not in st.session_state:
    st.info("👈 Click **Fetch & Refresh Data** to load data.")
    st.stop()
...
with tab2:
    st.markdown("### 🗺️ Regional Price Map 📈")
    ...
    if matched.empty:
        ...
        map_html, price_map = build_map_html(
            map_df, map_commodity, map_month_num, map_type
        )
        if map_html:
    ...
    # ── Prediction ────────────────────────────────────────────────────────────
    st.markdown("### 🔮 National Current Price Vs Prediction — Next Month")
    st.caption(
...
with tab3:
    ...
    with dl1:
        st.download_button(
            ...
            file_name=f"mpdata_{datetime.now().strftime('%Y%m%d')}.csv",
            mime="text/csv",
            use_container_width=True,
        )
    with dl2:
        ...
        st.download_button(
            "⬇️ Download Excel",
            data=xl.getvalue(),
            file_name=f"mpdata_{datetime.now().strftime('%Y%m%d')}.xlsx",
